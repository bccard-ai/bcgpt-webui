<script lang="ts">
	...
	interface Props {
		...
		show?: boolean;
		/** Callback invoked when archived chat data changes */
	...
	let chats = $state<ArchivedChat[]>([]);

	let searchValue = $state('');
	let showUnarchiveAllConfirmDialog = $state(false);

	const loadArchivedChats = async () => (await getArchivedChatList('')) as ArchivedChat[];

	const unarchiveChatHandler = async (chatId: string) => {
		await archiveChatById('', chatId).catch((error) => {
			toast.error(`${error}`);
		});

		chats = await loadArchivedChats();
		onchange?.();
	};

	const deleteChatHandler = async (chatId: string) => {
		await deleteChatById('', chatId).catch((error) => {
			toast.error(`${error}`);
		});
										...
										{#each chats.filter((c) => searchValue === '' || c.title
											...
											>
												<td class="px-3 py-1 w-2/3">
													<a href={resolve(`/c/${chat.id}`)} target="_blank">
