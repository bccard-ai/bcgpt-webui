<script lang="ts">
	...
	interface Props {
		...
		show?: boolean;
		/** Callback invoked when the form is submitted with channel data */
		onSubmit?: (data: { name: string; access_control: AccessControlValue }) => unknown;
		/** Callback invoked when the channel is updated or deleted */
		onUpdate?: () => unknown;
		/** Channel data for edit mode (null for create mode) */
	...
	let name = $state('');
	let accessControl = $state<AccessControlValue>(null);
	...
	const deleteHandler = async () => {
		...
		const res = await deleteChannelById('', channel.id).catch((error) => {
		...
		if (res) {
			...
			if (get(page).url.pathname === `/channels/${channel.id}`) {
				goto(resolve('/'));
			}
		}
