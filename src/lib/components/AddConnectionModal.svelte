<script lang="ts">
	...
	/**
	 * Modal dialog for adding or editing an API connection (OpenAI or Ollama).
	 * Supports URL configuration, API key entry, tags, prefix IDs, model ID management,
	 ...
	 * ```
	 *
	 * @param onSubmit - Callback with the connection data when saved.
	 * @param onDelete - Callback when the connection is deleted (edit mode).
	 * @param show - Bindable boolean controlling modal visibility.
	 * @param edit - Whether this is an edit of an existing connection.
	 * @param ollama - Whether the connection is for an Ollama server.
	 * @param direct - Whether to use direct connection mode.
	 * @param connection - Existing connection data for editing.
	 */
	interface Props {
		onSubmit?: (connection: Connection) => unknown;
		onDelete?: () => unknown;
		show?: boolean;
		edit?: boolean;
		ollama?: boolean;
	...
	const verifyOpenAIHandler = async () => {
		const res = await verifyOpenAIConnection('', url, key, direct).catch((error) => {
			toast.error(`${error}`);
		});
