<script lang="ts">
	...
	/**
	 * TagInput — inline input for adding tags with autocomplete suggestions.
	 *
	 * @example
	 * ```svelte
	 * <TagInput label="Add Tags" onAdd={(name) => addTag(name)} />
	 * ```
	 *
	 * @props label - Placeholder label shown when the input is hidden
	 * @props onAdd - Called with the tag name when submitted
	 */
	interface Tag {
		name: string;
	}
	...
	interface Props {
		/** Placeholder label shown when the input is hidden. */
		label?: string;
		/** Called with the tag name when submitted. */
	...
	{#if showTagInput}
		<div class="flex items-center">
			...
			<datalist id="tagOptions">
				{#each $tags as tag ((tag as Tag).name)}
					<option value={(tag as Tag).name}></option>
				{/each}
			</datalist>
