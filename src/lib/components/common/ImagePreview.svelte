<script lang="ts">
	...
	/**
	 * ImagePreview — full-screen image viewer with pan/zoom and download.
	 *
	 * @example
	 * ```svelte
	 * <ImagePreview bind:show src={imageUrl} alt="Preview" />
	 * ```
	 *
	 * @props show - Bindable visibility
	 * @props src - Image URL
	 * @props alt - Alt text (used as download filename prefix)
	 */
	interface Props {
		/** Bindable visibility. */
		show?: boolean;
		/** Image URL. */
		src?: string;
		/** Alt text. Used as prefix for download filename. */
		alt?: string;
	}
	...
	const downloadImage = (url: string, filename: string, prefixName = '') => {
		fetch(url)
