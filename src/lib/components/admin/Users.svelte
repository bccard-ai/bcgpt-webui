<script lang="ts">
	 ...
	 * Admin Users Page
	 *
	 * Tabbed container for user management: Overview (user list with CRUD)
	 * and Groups (user group management). Redirects non-admin users on mount.
	 */
	...
	/** Refresh users when tab changes */
	$effect(() => {
		if (selectedTab) {
			getUsersHandler();
		}
...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-6">
	<div
		...
		role="tablist"
		aria-label={$i18n.t('User management sections')}
	...
	>
		...
		<button
			role="tab"
			aria-selected={selectedTab === 'groups'}
			class="px-2 py-2 min-w-fit rounded-lg lg:flex-none flex text-right transition {selectedTab ===
			'groups'
				? ''
				: ' text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'}"
