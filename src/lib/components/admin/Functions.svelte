<script lang="ts">
	 ...
	 *
	 * Lists all custom plugin functions (filters, pipes, actions) with
	 * search, enable/disable toggle, global toggle, CRUD operations,
	 * import/export, share, and clone support. Shift-click shows quick delete.
	 */
	...
	import { APP_NAME_STORE, config, functions, models, settings } from '$lib/stores';
	...
	let selectedFunction = $state(null as FunctionItem | null);
	...
	/** Filter functions by query and sort by type then name */
	/** Refresh functions and models stores from backend */
	...
	/**
	 * Open the BCGPT Community share page and post the function data.
	 * @param func - Function item to share
	 */
	...
	/**
	 * Clone a function by storing it in sessionStorage and navigating to the create page.
	 * @param func - Function item to clone
	 */
	const cloneHandler = async (func) => {
		...
		if (_function) {
			...
			goto(resolve('/admin/functions/create'));
		}
	};

	/**
	 * Export a single function as a JSON file download.
	 * @param func - Function item to export
	 */
	...
	/**
	 * Delete a function and refresh stores.
	 * @param func - Function item to delete
	 */
	...
	/**
	 * Toggle the global status of a function and refresh stores.
	 * @param func - Function item to toggle
	 */
...
<svelte:head>
	<title>
		{$i18n.t('Functions')} | {$APP_NAME_STORE}
	</title>
</svelte:head>

<div class="flex flex-col gap-1.5 mt-2 mb-3">
	<InfoCallout variant="warning"
		>{$i18n.t(
			'Manage custom plugin Functions (filters, pipes, and actions) that extend chat behavior. Functions run arbitrary code, so only install them from sources you trust.'
		)}</InfoCallout
	>
	...
	<div class=" flex w-full space-x-2">
		...
		<div>
			<a
				...
				href={resolve('/admin/functions/create')}
			>
...
<div class="mb-5">
	{#if filteredItems.length === 0}
		<EmptyState
			...
			description={$i18n.t('Create a function or import one to get started.')}
			actionLabel={$i18n.t('Create Function')}
			actionHandler={() => {
				goto(resolve('/admin/functions/create'));
			}}
		/>
	{:else}
		{#each filteredItems as func (func.id)}
			<div
				class=" flex space-x-4 cursor-pointer w-full px-4 py-3 dark:hover:bg-white/5 hover:bg-black/5 rounded-xl"
			>
				<a
					class=" flex flex-1 space-x-3.5 cursor-pointer w-full"
					href={resolve(`/admin/functions/edit?id=${encodeURIComponent(func.id)}`)}
				>
				...
				<div class="flex flex-row gap-0.5 self-center">
					...
					{:else}
						...
						<FunctionMenu
							{func}
							editHandler={() => {
								goto(resolve(`/admin/functions/edit?id=${encodeURIComponent(func.id)}`));
							}}
							shareHandler={() => {
								shareHandler(func);
							}}
...
<div class=" flex justify-end w-full mb-2">
	<div class="flex space-x-2">
		<input
			...
			accept=".json"
			hidden
			/** Handle file import selection — show confirmation dialog */
			onchange={() => {
				showConfirm = true;
			}}
		/>
	...
	onconfirm={() => {
		...
		reader.onload = async (event) => {
			...
			for (const func of _functions) {
				await createNewFunction('', func).catch((error) => {
					toast.error(`${error}`);
					return null;
				});
			}
