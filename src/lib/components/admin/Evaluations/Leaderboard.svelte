<script lang="ts">
	/**
	 * Evaluation Leaderboard Tab
	 *
	 * Ranks models by Elo rating computed from arena feedback.
	 * Supports semantic re-ranking via tag embedding similarity search
	 * using a lightweight HuggingFace transformer model.
	 */
	...
	/** HuggingFace embedding model for tag similarity */
	...
	const getTransformers = async () => {
		...
		const { env, AutoModel, AutoTokenizer } = await transformersModule;
	...
	let rankedModels = $state([] as Record<string, unknown>[]);
	let query = $state('');

	/** Non-reactive embedding cache keyed by tag string */
	let tagEmbeddings = new Map(); // eslint-disable-line svelte/prefer-svelte-reactivity -- non-reactive embedding cache
	...
	/**
	 * Compute Elo ratings and rank models.
	 * @param similarities - Optional map of feedback ID → similarity score for topic filtering
	 */
	...
	function calculateModelStats(
		...
		const stats = new Map<string, ModelStats>(); // eslint-disable-line svelte/prefer-svelte-reactivity -- local computation, not reactive state
		const K = 32;
	...
	/** Find the maximum cosine similarity between a query embedding and a set of tag embeddings */
	...
	/** Load the HuggingFace tokenizer and model, pre-compute tag embeddings */
	...
	const getTagEmbeddings = async (tags: string[]) => {
		const embeddings = new Map(); // eslint-disable-line svelte/prefer-svelte-reactivity -- local computation, not reactive state
	...
	const debouncedQueryHandler = async () => {
		...
		debounceTimer = setTimeout(async () => {
			...
			const similarities = new Map<string, number>(); // eslint-disable-line svelte/prefer-svelte-reactivity -- local computation, not reactive state
...
<InfoCallout
	>{$i18n.t(
		'This leaderboard ranks models by Elo rating computed from user feedback in arena comparisons, helping you gauge relative model performance.'
	)}</InfoCallout
>
