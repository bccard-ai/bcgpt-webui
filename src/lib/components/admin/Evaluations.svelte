<script lang="ts">
	 ...
	 *
	 * Tabbed container for the evaluation leaderboard and feedback history.
	 * Fetches all feedbacks on mount and passes them to child tabs.
	 */
...
{#if loaded}
	<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-6">
		<div
			...
			role="tablist"
			aria-label={$i18n.t('Evaluation sections')}
		...
		>
			...
			<button
				role="tab"
				aria-selected={selectedTab === 'feedbacks'}
				class="px-2 py-2 min-w-fit rounded-lg lg:flex-none flex text-right transition {selectedTab ===
				'feedbacks'
					? ''
					: ' text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'}"
