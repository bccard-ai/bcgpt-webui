<script lang="ts">
	 ...
	 * Edit Group Modal
	 *
	 * Tabbed modal for editing an existing group or creating a new one.
	 * Tabs: General (name/description), Permissions, Users.
	 * Also used for editing default permissions (custom=false).
	 */
	...
	interface Props {
		onSubmit?: (group: Record<string, unknown>) => Promise<void>;
		onDelete?: () => void;
		show?: boolean;
		edit?: boolean;
	...
	/**
	 * Submit the group form and close the modal.
	 */
	...
	/** Populate form fields from the existing group data */
	const init = () => {
		if (group) {
			name = group.name;
