<script lang="ts">
	/**
	 * Group Users Selector
	 *
	 * Searchable checkbox list of non-admin users for group membership.
	 * Selected users appear first, sorted by selection order.
	 */
					...
					<div class="flex flex-row items-center gap-3 w-full text-sm">
						...
						<div class="flex w-full items-center justify-between">
							...
							{#if userIds.includes(user.id)}
								<Badge type="success" content={$i18n.t('member')} />
							{/if}
						</div>
					</div>
				{/each}
