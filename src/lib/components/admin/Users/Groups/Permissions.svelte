<script lang="ts">
	 ...
	 *
	 * Toggle-based editor for workspace, sharing, chat, and feature
	 * permissions. Fills missing permission keys from defaults on mount
	 * and reactively.
	 */
	...
	/** Default permission values used to fill missing keys */
	...
	type PermissionGroups = Record<string, Record<string, boolean>>;

	/**
	 * Merge user permissions with defaults so all keys are present.
	 * @param obj - Current permissions (may have missing keys)
	 * @param defaults - Full default permissions structure
	 */
	function fillMissingProperties(obj: PermissionGroups, defaults: PermissionGroups) {
		return {
			...defaults,
			...obj,
	...
	<!-- <div>
					...
					<div class="flex flex-col w-full">
						...
						{#if model_ids.length > 0}
							<div class="flex flex-col">
								{#each model_ids as modelId, modelIdx (modelId)}
		...
		<div class=" space-y-1 mb-3">
			...
			<div class="flex-1 mr-2">
				<select
					class="w-full border-1 border-gray-200 p-1 rounded-md focus:text-blue-700 px-2 text-sm"
