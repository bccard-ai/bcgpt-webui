	...
	const submitHandler = async () => {
		...
		if (tab === '') {
			...
			if (inputFiles) {
				...
				// CSV import is async; loading is cleared by stopLoading() in
				// reader.onload. Return so the synchronous `loading = false`
				// below doesn't clear it before the import has run.
				return;
			} else {
							...
							<div>
								...
								<div class=" text-xs text-gray-500">
									...
									<a
										...
										href={resolve('/static/user-import.csv' as unknown as '/')}
									>
