<script lang="ts">
	 ...
	 *
	 * Settings modal for model ordering, default model selection, and bulk reset.
	 * Provides drag-and-drop reordering via ModelList, default model picker,
	 * and a destructive "Reset All Models" action with confirmation.
	 */
	...
	interface Props {
		...
		show?: boolean;
		/** Callback to refresh the parent model list after changes */
		initHandler?: () => void;
	}
	...
	/** Currently selected model in the default model dropdown */
	let selectedModelId = $state('');
	/** Ordered list of default model IDs */
	let defaultModelIds = $state([]);
	/** Full ordered list of all model IDs */
	let modelIds = $state([]);
	...
	/** Whether the destructive reset confirmation is showing */
	let showResetModal = $state(false);

	/**
	 * Add the currently selected model to the default models list.
	 * Ignores empty selections and duplicates.
	 */
	const onModelSelect = () => {
	...
	/**
	 * Load model configuration from the backend.
	 * Parses DEFAULT_MODELS (comma-separated) and MODEL_ORDER_LIST,
	 * then merges with all known model IDs alphabetically.
	 */
	const init = async () => {
		config = await getModelsConfig('');
		...
		const allModelIds = get(models).map((model) => model.id);
	...
	/**
	 * Persist model ordering and default models to the backend.
	 * Shows success/error toast and closes the modal on success.
	 */
	...
	/**
	 * Sort the model list by model name in ascending or descending order.
	 * Toggles direction if the same sort key is clicked again.
	 */
	const sortByModel = () => {
		sortKey = 'model';
	...
	/**
	 * Delete all models after user confirmation.
	 * Shows success/error toast and refreshes parent.
	 */
	const resetAllModels = async () => {
		const res = await deleteAllModels('').catch((error) => {
			toast.error(`${error}`);
			return null;
		});
		if (res) {
	...
	/** Auto-add selected default model */
	$effect(() => {
		if (selectedModelId) {
			onModelSelect();
		}
					...
					>
						<div>
							<div class="flex flex-col w-full">
								<button class="mb-1 flex gap-2" type="button" onclick={sortByModel}>
						...
						<div>
							<div class="flex flex-col w-full">
								...
								<div class="flex items-center -mr-1">
									<select
										class="w-full py-1 text-xs rounded-lg bg-transparent {selectedModelId
											? ''
								...
								{#if defaultModelIds.length > 0}
									<div class="flex flex-col">
										{#each defaultModelIds as modelId, modelIdx (modelIdx)}
