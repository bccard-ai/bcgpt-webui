<script lang="ts">
	 ...
	 *
	 * Full Ollama model management interface: pull models from Ollama.com,
	 * delete local models, create custom models from JSON specs, and upload
	 * GGUF model files. Supports parallel downloads with progress tracking
	 * and cancellation via MODEL_DOWNLOAD_POOL.
	 */
	...
	interface Props {
		/** Index of the Ollama URL to manage, or null for default */
		urlIdx?: number | null;
	}
	...
	/**
	 * Parse a streaming response from Ollama, updating progress and showing toasts.
	 * @param reader - The readable stream reader
	 * @param onDigest - Callback when digest progress is received (completed, total)
	 * @param onStatus - Callback for non-digest status messages
	 */
	const parseOllamaStream = async (reader, onDigest, onStatus) => {
		while (true) {
			try {
				...
				// Stream parse errors are non-fatal
			}
		}
	};

	/**
	 * Re-pull all installed Ollama models to update them to latest versions.
	 * Processes sequentially, showing per-model progress.
	 */
	const updateModelsHandler = async () => {
		for (const model of ollamaModels) {
			updateModelId = model.id;
			const [res, _controller] = await pullModel('', model.id, urlIdx).catch((error) => {
				toast.error(`${error}`);
				return [null, null];
			});
	...
	/**
	 * Pull (download) a model from Ollama.com by tag.
	 * Registers the download in MODEL_DOWNLOAD_POOL for progress tracking
	 * and cancellation support.
	 */
	const pullModelHandler = async () => {
		const sanitizedModelTag = modelTag.trim().replace(/^ollama\s+(run|pull)\s+/, '');

		if (get(MODEL_DOWNLOAD_POOL)[sanitizedModelTag]) {
			toast.error(
		...
		if (Object.keys(get(MODEL_DOWNLOAD_POOL)).length === MAX_PARALLEL_DOWNLOADS) {
			toast.error(
	...
	/**
	 * Upload a GGUF model file or download from URL, then create a model.
	 * Handles both file upload and URL download modes with progress tracking.
	 */
	const uploadModelHandler = async () => {
		...
		if (modelUploadMode === 'file') {
			...
			if (file) {
				uploadMessage = $i18n.t('Uploading...');
	...
	/**
	 * Delete a local Ollama model by tag after user confirmation.
	 */
	...
	/**
	 * Cancel an in-progress model download, abort the stream, and clean up.
	...
	/**
	 * Create a new model from a JSON spec. Parses the spec, sends to Ollama,
	 * and tracks creation progress.
	 */
					...
					for (const line of lines) {
						...
						if (data.status) {
							if (
								...
								createModelPullProgress = data.completed
									? Math.round((data.completed / data.total) * 1000) / 10
									: 100;
							}
						}
	...
	const init = async () => {
		loading = true;
		ollamaModels = await getOllamaModels('', urlIdx).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
	...
	/** Re-fetch models when the URL index changes */
	$effect(() => {
		if (urlIdx !== null) {
			init();
		}
			...
			<div class="space-y-2">
				...
				<div>
					...
					<div class="flex w-full">
						...
						<button
							class="px-2.5 bg-gray-50 hover:bg-gray-200 text-gray-800 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-100 rounded-lg transition"
							aria-label={$i18n.t('Delete a model')}
							onclick={() => {
					...
					<div class="flex w-full">
						<div class="flex-1 mr-2 flex flex-col gap-2">
							...
							<textarea
								...
								class="w-full rounded-lg py-1.5 px-3 text-xs bg-gray-50 dark:text-gray-100 dark:bg-gray-850 outline-hidden resize-none scrollbar-hidden"
								rows="6"
				...
				{#if showExperimentalOllama}
					...
					>
						<div
							class=" mb-2 flex w-full justify-between pb-2 border-b-1 border-gray-100 dark:border-gray-700 border-dashed pb-2 border-b-1 border-gray-100 dark:border-gray-700 border-dashed"
						>
							...
							<div>
								<div>
									...
									<textarea
										...
										class="w-full rounded-lg py-1.5 px-3 text-xs bg-gray-50 dark:text-gray-100 dark:bg-gray-850 outline-hidden resize-none"
										rows="6"
									></textarea>
								</div>
							</div>
