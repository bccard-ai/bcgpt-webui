<script lang="ts">
	...
	interface Props {
		/** Available action function definitions */
		actions?: unknown[];
		/** IDs of currently selected actions (two-way bindable) */
		selectedActionIds?: string[];
	}

	let { actions = [], selectedActionIds = $bindable([]) }: Props = $props();
	...
	<div class="flex flex-col">
		{#if actions.length > 0}
			<div class=" flex items-center mt-2 flex-wrap">
				{#each Object.keys(_actions) as action (action)}
