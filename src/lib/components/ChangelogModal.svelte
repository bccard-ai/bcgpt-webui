<script lang="ts">
	...
	import { APP_NAME_STORE, config, settings } from '$lib/stores';
	...
	interface ChangelogVersion {
		date: string;
		[key: string]: ChangelogSection[] | string;
	}

	/**
	 * Modal dialog displaying the changelog for the current and past releases.
	 * Shows version details organized by section (added, fixed, changed, removed)
	 * with confetti celebration on open.
	 *
	 * @example
	 * ```svelte
										...
										{#each Object.keys(changelog[version][section]) as item (item)}
											<div class="text-sm mb-2">
												<div class="font-semibold uppercase">
													{(changelog[version][section] as ChangelogSection[])[item].title}
												</div>
												<div class="mb-2 mt-1">{(changelog[version][section] as ChangelogSection[])[item].content}</div>
											</div>
										{/each}
									</div>
								</div>
