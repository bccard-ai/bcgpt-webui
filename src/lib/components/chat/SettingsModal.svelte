<script lang="ts">
	...
	import SkillsPanel from './Settings/Skills.svelte';
	import McpPanel from './Settings/Mcp.svelte';
	...
	interface Props {
		/** Whether the settings modal is visible. Bindable. */
		show?: boolean;
	}
	...
	/** Search query for filtering settings tabs. */
	let search = $state('');
	/** List of tab IDs currently visible based on search filter. */
	let visibleTabs = $state(searchData.map((tab) => tab.id));
	let searchDebounceTimeout: ReturnType<typeof setTimeout>;

	/**
	 * Filter settings tabs by matching query against tab titles and keywords.
	 * @param query - The search string to filter by.
	 * @returns Array of matching tab IDs.
	 */
	...
	/** Debounced search handler — updates visibleTabs after 100ms of inactivity. */
	...
	/**
	 * Persist updated settings to the store and backend.
	 * Also refreshes the models list to reflect any configuration changes.
	 * @param updated - Partial settings object to merge into current settings.
	 */
	...
	/** Fetch the current models list, respecting direct connections config. */
	...
	/** Currently active settings tab ID. */
	let selectedTab = $state('general');

	/**
	 * Handle sideways scrolling on the tabs container by converting
	 * vertical wheel events into horizontal scroll.
	 */
	const scrollHandler = (event: Event) => {
		const settingsTabsContainer = document.getElementById('settings-tabs-container');
		if (settingsTabsContainer) {
			event.preventDefault();
			settingsTabsContainer.scrollLeft += (event as WheelEvent).deltaY;
		}
	};

	/** Attach the horizontal scroll listener after the DOM updates. */
	const addScrollListener = async () => {
		await tick();
		document.getElementById('settings-tabs-container')?.addEventListener('wheel', scrollHandler);
	};

	/** Detach the horizontal scroll listener. */
	const removeScrollListener = async () => {
		await tick();
		document.getElementById('settings-tabs-container')?.removeEventListener('wheel', scrollHandler);
	};

	/** Select a settings tab by ID. */
	const selectTab = (tabId: string) => {
		selectedTab = tabId;
	};

	/**
	 * Navigate to the admin settings page and close this modal.
	...
	/**
	 * Save-and-toast wrapper — persists settings and shows a success toast.
	 * Used by Connections, Tools, and other tabs that need immediate feedback.
	 */
	const saveSettingsWithToast = async (updated: Record<string, unknown>) => {
		...
		<div class="flex flex-col md:flex-row w-full px-4 pt-1 pb-4 md:space-x-4">
				...
				{#if visibleTabs.length > 0}
					{#each visibleTabs as tabId (tabId)}
						...
						{:else if tabId === 'skills'}
							...
							>
								<div class="self-center mr-2 text-sm">✦</div>
			...
			<div class="flex-1 md:min-h-[32rem] max-h-[32rem]">
				...
				{:else if selectedTab === 'connections'}
					<Connections saveSettings={saveSettingsWithToast} />
				{:else if selectedTab === 'tools'}
					<Tools saveSettings={saveSettingsWithToast} />
				{:else if selectedTab === 'skills'}
					<SkillsPanel saveSettings={saveSettingsWithToast} />
				{:else if selectedTab === 'mcp'}
					<McpPanel saveSettings={saveSettingsWithToast} />
