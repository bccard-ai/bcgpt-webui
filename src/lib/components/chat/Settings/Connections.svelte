<script lang="ts">
	...
	type DirectConnectionsConfig = {
		...
		OPENAI_API_CONFIGS: Record<number, Record<string, unknown>>;
	};
	...
	let config: DirectConnectionsConfig | null = $state(null);
	...
	onMount(async () => {
		config = get(settings)?.directConnections ?? {
				...
				<div class="pr-1.5">
					<div class="">
						...
						<div class="flex flex-col gap-1.5">
							{#each config?.OPENAI_API_BASE_URLS ?? [] as url, idx (url)}
								<Connection
