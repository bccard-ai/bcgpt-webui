<script lang="ts">
	...
	/** Status data for a web search operation */
	interface WebSearchStatus {
		urls: string[];
		query: string;
	}

	/** Props for the WebSearchResults component - displays collapsible web search results */
	interface Props {
		/** Current web search status with URLs and query */
		status?: WebSearchStatus;
		/** Custom content rendered in the collapsible header */
		children?: Snippet;
	}
...
<Collapsible bind:open={isOpen} className="w-full space-y-1">
	...
	>
		...
		{#each status.urls as url, urlIdx (urlIdx)}
			<a
				href={url}
				rel="external"
				target="_blank"
