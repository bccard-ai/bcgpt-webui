<script lang="ts">
	...
	interface Props {
		...
		messageId: string;
		siblings: string[];
		gotoMessage: (message: Record<string, unknown>, index: number) => void;
		showPreviousMessage: (message: Record<string, unknown>) => void;
		showNextMessage: (message: Record<string, unknown>) => void;
	...
	/** Enters edit mode for the message */
	...
	/** Confirms the edit and optionally submits */
	async function editMessageConfirmHandler(submit = true): Promise<void> {
	...
	/** Resolves the profile image source for the current message */
	function resolveProfileSrc(): string {
		if (message.user) {
			return (
				$models.find((m) => m.id === message.user)?.info?.meta?.profile_image_url ?? '/user.png'
			);
		}
		return (user?.profile_image_url as string | undefined) ?? '/user.png';
	}

	/** Resolves the display name for the message sender */
...
<div class="flex w-full user-message" dir={$settings.chatDirection} id="message-{message.id}">
	{#if !($settings?.chatBubble ?? true)}
		<div class={`shrink-0 ${($settings?.chatDirection ?? 'LTR') === 'LTR' ? 'mr-3' : 'ml-3'}`}>
			<ProfileImage src={resolveProfileSrc()} className="size-8" />
		</div>
	{/if}
		...
		<div class="chat-{message.role} w-full min-w-full markdown-prose">
			{#if message.files}
				<div class="mt-2.5 mb-1 w-full flex flex-col justify-end overflow-x-auto gap-1 flex-wrap">
					{#each message.files as file (file.id ?? file.url ?? file.name)}
								...
								{#if siblings.length > 1}
									<div class="flex self-center" dir="ltr">
										...
										{:else}
											<!-- svelte-ignore a11y_no_static_element_interactions -->
											<div
												class="text-sm tracking-widest font-semibold self-center dark:text-gray-101 min-w-fit"
												ondblclick={async () => {
													messageIndexEdit = true;
