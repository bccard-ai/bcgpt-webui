<script lang="ts">
	...
	let tools = $state<Record<string, ToolMenuItem>>({});
	let show = $state(false);

	let fileUploadEnabled = $derived(
		get(user).role === 'admin' || get(user)?.permissions?.chat?.file_upload
	);
	...
	const init = async () => {
		...
		tools = (get(_tools) as unknown as ToolModel[]).reduce(
			(a: Record<string, ToolMenuItem>, tool: ToolModel) => {
				a[tool.id] = {
					name: tool.name,
									...
									<div class=" shrink-0">
										<Switch
											...
											onchange={async (e: unknown) => {
												const state = (e as CustomEvent | undefined)?.detail;
												await tick();
												if (state) {
