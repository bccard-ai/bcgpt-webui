<script lang="ts">
	...
	const startRecording = async () => {
		...
		mediaRecorder.onstop = async () => {
			if (
				get(config).audio.stt.engine === 'web' ||
				(get(settings)?.audio?.stt?.engine ?? '') === 'web'
			) {
				audioChunks = [];
			} else {
				if (confirmed) {
		...
		if (
			...
			if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
				...
				speechRecognition.onerror = function (event: Event) {
...
>
	...
	>
		...
		>
			{#each visualizerData.slice().reverse() as rms, i (i)}
