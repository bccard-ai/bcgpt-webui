<script lang="ts">
	...
	interface Props {
		...
		onUrl?: (...args: unknown[]) => void;
		onYoutube?: (...args: unknown[]) => void;
	}
	...
	let filteredItems = $derived(
		fuse
			? command.slice(1)
				? fuse.search(command).map((e: { item: unknown }) => {
						return e.item;
					})
				: items
			: []
	...
	onMount(() => {
		let legacy_documents = get(knowledge)
