<script lang="ts">
	...
	import { chats, settings, user as _user, temporaryChatEnabled } from '$lib/stores';
	import { tick, getContext } from 'svelte';
	import { toast } from 'svelte-sonner';
	import { updateChatById, updateChatEntryInList } from '$lib/apis/chats';
	...
	interface Props {
		...
		atSelectedModel: unknown;
		sendPrompt: (...args: unknown[]) => unknown;
		continueResponse: (...args: unknown[]) => unknown;
		regenerateResponse: (...args: unknown[]) => unknown;
		mergeResponses: (...args: unknown[]) => unknown;
		chatActionHandler: (...args: unknown[]) => unknown;
		showMessage?: (...args: unknown[]) => unknown;
		submitMessage?: (...args: unknown[]) => unknown;
	...
	let {
		...
		chatId = '',
		user = get(_user),
		// False positive: `prompt` is a two-way ($bindable) prop written here and
		// read by the parent (Chat.svelte's bind:prompt). no-useless-assignment's
		// single-file flow analysis can't see the cross-component read.
	...
	const updateChat = async () => {
		if (!get(temporaryChatEnabled)) {
			...
			chats.set(updateChatEntryInList(get(chats), chatId, { updated_at: Date.now() / 1000 }));
		}
	};
	...
	const gotoMessage = async (message, idx) => {
		...
		if (get(settings)?.scrollOnBranchChange ?? true) {
			const element = document.getElementById('messages-container');
	...
	$effect(() => {
		if (history.currentId) {
			let _messages: HistoryMessage[] = [];

			let message: HistoryMessage | null = history.messages[history.currentId];
				...
				<div class="w-full">
					...
					{#each messages as message, messageIdx (message.id)}
						<Message
							...
							{showNextMessage}
							{updateChat}
							editMessage={editMessage as (...args: unknown[]) => void}
							{deleteMessage}
							{rateMessage}
							actionMessage={actionMessage as (...args: unknown[]) => void}
							{saveMessage}
							{submitMessage}
							{regenerateResponse}
							{continueResponse}
