<script lang="ts">
	...
	/**
	 * Renders a single channel message with support for editing, deleting,
	 * reactions, threading, file attachments, and profile previews.
	 *
	 * @example
	 * ```svelte
	 * <Message
	 ...
	 *   onReaction={(name) => toggleReaction(msg.id, name)}
	 * />
	 * ```
	 *
	 * @param message - The message object to render.
	 * @param showUserProfile - Whether to show the user profile header for this message.
	 * @param thread - Whether this message is rendered inside a thread view.
	 * @param onDelete - Callback invoked when the message is deleted.
	 * @param onEdit - Callback invoked with the new content when the message is edited.
	 * @param onThread - Callback invoked with the message ID to open a thread.
	 * @param onReaction - Callback invoked with the reaction name when a reaction is toggled.
	 */
	interface Props {
...
{#if message}
	...
	>
					...
					{#if message.user_id === $user.id || $user.role === 'admin'}
						<Tooltip content={$i18n.t('Edit')}>
							<button
								...
								onclick={() => {
									edit = true;
									editedContent = message.content as string;
								}}
							>
								<Pencil />
							</button>
		...
		>
			...
			>
				{#if showUserProfile}
					{@const messageUser = message.user as MessageUser | null | undefined}
					<ProfilePreview user={messageUser}>
						<ProfileImage
							src={messageUser?.profile_image_url ??
			...
			<div class="flex-auto w-0 pl-1">
				...
				{#if (message?.data?.files ?? []).length > 0}
					<div class="my-2.5 w-full flex overflow-x-auto gap-2 flex-wrap">
						{#each message?.data?.files as file (file.url ?? file.name)}
							<div>
								{#if file.type === 'image'}
