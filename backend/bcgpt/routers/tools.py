...
from bcgpt.models.tools import (
    ToolForm,
    ToolModel,
    ToolResponse,
    ToolUserResponse,
...
from bcgpt.utils import load_tools_module_by_id, replace_imports
from bcgpt.config import CACHE_DIR
from bcgpt.constants import ERROR_MESSAGES
from fastapi import APIRouter, Depends, HTTPException, Request, status
from bcgpt.utils import get_tools_specs
from bcgpt.utils import get_admin_user, get_verified_user
from bcgpt.utils import has_access, has_permission
from bcgpt.env import SRC_LOG_LEVELS, TOOLS_ALLOW_NON_ADMIN_CODE
...
def ensure_can_author_tool_code(user):
    """Authoring tool code is RCE-equivalent: the content is exec()'d without
    sandboxing (utils/plugin.py), so it runs with full process privileges.
    Restrict it to admins unless TOOLS_ALLOW_NON_ADMIN_CODE is explicitly set.
    The per-resource permission/ownership checks still apply on top of this."""
    if user.role != "admin" and not TOOLS_ALLOW_NON_ADMIN_CODE:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
                "Authoring tool code is restricted to administrators because tool "
                "code runs without sandboxing. Set TOOLS_ALLOW_NON_ADMIN_CODE=true "
                "to allow users with the 'workspace.tools' permission to author tools."
            ),
        )
...
async def create_new_tools(
    ...
    ensure_can_author_tool_code(user)
