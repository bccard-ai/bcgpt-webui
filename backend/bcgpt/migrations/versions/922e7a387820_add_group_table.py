"""Add group table and access_control columns.

Revision ID: 922e7a387820
Revises: 4ace53fd72c8
Create Date: 2024-11-14 03:00:00.000000

* Creates the ``group`` table for user group management.
* Adds ``access_control`` (JSON) to model, knowledge, prompt, and tool.
* Adds ``is_active`` (Boolean) to the model table.
"""
...
def upgrade() -> None:
    """Create the group table and add access_control columns."""
    op.create_table(
        "group",
...
def downgrade() -> None:
    """Drop the group table and remove access_control columns."""
    op.drop_table("group")
